...
// streamLargeDeltasAsRandomChunks previously split provider deltas into tiny delayed chunks to simulate
// a typewriter effect. For realtime streaming, forward each provider delta immediately.
async function* streamLargeDeltasAsRandomChunks(
	iterator: AsyncGenerator<TextStreamUpdate>
): AsyncGenerator<TextStreamUpdate> {
	for await (const textStreamUpdate of iterator) {
		yield textStreamUpdate;
		if (textStreamUpdate.done) return;
	}
}
